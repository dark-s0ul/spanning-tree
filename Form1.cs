...
namespace WindowsFormsApplication4 {
    public partial class Form1 : Form {
        ...
        private void control() {
            ...
            if(flag == false) {
                MessageBox.Show("Введене вами значення має некоректний формат", "Помилка");
                return;
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e) {
            if(MessageBox.Show("Ви впевненні що хочете очистити матрицю?", "Пітвердіть вибір", MessageBoxButtons.YesNo) == DialogResult.Yes) {
                for(short i = 0; i < n; i++) {
                    for(short j = 0; j < n; j++) {
                        if(i != j) {
